function [VU, Ap] = MaxBellmanU(par,be,bu,Grid)
...
D = min(f(par,Grid.AA,par.b) - 1e-3, Grid.A(end)); % -1e-3 so we always have positve consumption.

B = p*K+(1-p)*D;
C = (1-p)*K + p * D;


fB = BellmanU(par,be,bu,Grid.AA,Grid.YY,B);
fC = BellmanU(par,be,bu,Grid.AA,Grid.YY,C);
...
for it_inner = 1:MAXIT
    ...
    fC(I) = fB(I);
    B(I) = p*C(I) + (1-p)*K(I);
    fB(I) = BellmanU(par,be,bu,Grid.AA(I),Grid.YY(I),B(I));

    K(~I) = B(~I);
    B(~I) = C(~I);
    fB(~I) = fC(~I);
    C(~I) = p*B(~I) + (1-p)*D(~I);
    fC(~I) = BellmanU(par,be,bu,Grid.AA(~I),Grid.YY(~I),C(~I));
...
% value function.
VU = BellmanU(par,be,bu,Grid.AA,Grid.YY,Ap);
